  ...
  private buildDocument(input: {
    ...
    if (credentials.length > 0) {
      content.push(
        ...
        {
          text: 'The following credentials were configured for this project. Secret values are stored encrypted in Helm and are intentionally not included in this document.',
          style: 'caption',
          margin: [0, 4, 0, 8],
        },
        {
    ...
    return {
      ...
      footer: (currentPage: number, pageCount: number) => ({
        columns: [
          {
            text: `Generated by Helm · ${this.formatDate(new Date())}`,
            style: 'footer',
          },
          {
